...
app.use(async (req, res, next) => {
    ...
    // Canonical domain: force the bare apex (sitesawa.com) to www.sitesawa.com.
    // Without this, a token saved on one host is invisible on the other, which
    // makes the dashboard fail to find the login token. Only redirect the apex —
    // never subdomains, custom domains, or localhost.
    const hostNoPort = host.split(':')[0];
    if (hostNoPort === 'sitesawa.com') {
        return res.redirect(301, 'https://www.sitesawa.com' + req.originalUrl);
    }
